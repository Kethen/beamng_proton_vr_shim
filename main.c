...
#define HOOK_API2(mod_name, name) { \
	HANDLE lib_handle = LoadLibraryW(mod_name); \
	if (lib_handle == NULL){ \
	...
	int create_status = MH_CreateHook(target, name##_hooked, (void **)&name##_orig); \
	if (create_status != MH_OK){ \
		LOG("%s: failed creating hook for %ls %s, %d\n", __func__, mod_name, STR(name), create_status); \
		exit(1); \
	} \
...
static int WINAPI (*xrGetVulkanDeviceExtensionsKHR_orig)(void *instance, void *systemId, uint32_t bufferCapacityInput, uint32_t *bufferCountOutput, char *buffer) = NULL;
...
int WINAPI (*xrGetInstanceProcAddr_orig)(void *instance, const char *fn_name, void **out_fn) = NULL;
int WINAPI xrGetInstanceProcAddr_hooked(void *instance, const char *fn_name, void **out_fn){
	...
	if (result == 0 && strcmp(fn_name, "xrGetVulkanDeviceExtensionsKHR") == 0){
		xrGetVulkanDeviceExtensionsKHR_orig = *out_fn;
		*out_fn = xrGetVulkanDeviceExtensionsKHR_hooked;
	}
	return result;
}
...
int init(){
	...
	CreateDXGIFactory1_real = (void *)GetProcAddress(real_dll, "CreateDXGIFactory1");
